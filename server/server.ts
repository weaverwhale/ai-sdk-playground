...
app.post('/api/deep-search', async (req, res) => {
  try {
    ...
    console.error('[SERVER] Deep search API error:', error instanceof Error ? error.message : String(error));
    res.status(500).json({ 
...
app.post('/api/execute-deep-search', async (req, res) => {
  try {
    ...
    if (availableProviders.length === 0) {
      console.error('[SERVER] Execute deep search API - No model providers available. Please check your API keys in .env file');
...
app.get('/api/deep-search-status', async (req, res) => {
  try {
    ...
    if (!plan && allPlans.length > 0) {
      console.log(`[SERVER] Client requested plan ${planId} but it doesn't exist directly. Looking for matches.`);
      ...
      if (!plan) {
        ...
        if (isTimestampBased) {
          ...
          if (closestMatch && closestDistance < 100000) { // Accept matches within reasonable tolerance
            console.log(`[SERVER] Found closest timestamp match: ${closestMatch} (distance: ${closestDistance}) for request: ${planId}`);
    ...
    console.log(`[SERVER] Deep search status API - Plan ${planId} steps:`, 
      plan.steps.map(step => ({
        id: step.id,
        status: step.status,
    ...
    console.log(`[SERVER] Plan status summary: ${Object.entries(statusSummary).map(([status, count]) => `${status}: ${count}`).join(', ')}`);
