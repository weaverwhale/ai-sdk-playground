...
export function chatReducer(state: ChatState, action: ChatAction): ChatState {
  switch (action.type) {
    ...
    case 'UPDATE_PLAN_STEP': {
      ...
      console.log(`[CHAT REDUCER] Updating step ${action.payload.stepId} to ${action.payload.status}`);
      ...
      const updatedSteps = state.searchPlan.steps.map(step => {
        if (step.id === action.payload.stepId) {
          console.log(`[CHAT REDUCER] Found step to update: ${step.id} from ${step.status} to ${action.payload.status}`);
      ...
      console.log(`[CHAT REDUCER] Plan updated, returning new state with ${updatedSteps.length} steps`);
    ...
    case 'UPDATE_SEARCH_PLAN': {
      ...
      // Debug: log all step statuses to verify they're updated
      const stepStatuses = action.plan.steps.map((step: any) => `${step.id}: ${step.status}`).join(', ');
      console.log(`[CHAT REDUCER] Updated plan steps: ${stepStatuses}`);
